...
strength$Station <- with(stations, Nome[match(strength$Station, Codice)])
...
# Outliers interpreted by line
st_func_df$out_group <- 'Normal'
st_func_df[st_func_df$Station %in% c("Pozzuolo Martesana","Melzo"), "out_group"] <- 'R4'
st_func_df[st_func_df$Station %in% c("Arcore", "Carnate Usmate", "Paderno Robbiate", "Calusco","Terno" ), "out_group"] <- 'R14'
st_func_df[st_func_df$Station %in% c("Milano Porta Garibaldi"), "out_group"] <- 'Milano PG'
